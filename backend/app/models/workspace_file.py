...
class WorkspaceFolder(Base):
    """A single level of grouping for a grant's workspace files.

    One level deep only — a folder holds files, never other folders. Files with
    no folder are "loose" (folder_id is NULL) and show at the front of the panel.
    """
...
class WorkspaceFile(Base):
    ...
    id: Mapped[str] = mapped_column(String, primary_key=True, default=lambda: str(uuid.uuid4()))
    grant_id: Mapped[str] = mapped_column(String, ForeignKey("active_grants.id"), nullable=False, index=True)
    folder_id: Mapped[str | None] = mapped_column(String, ForeignKey("workspace_folders.id", ondelete="SET NULL"), index=True)
