...
class ActiveGrant(Base):
    ...
    # Expert-reviewer async job state + macro summary (verdict/score/strengths/risks)
    ai_review_status: Mapped[str] = mapped_column(String(20), default="idle", server_default="idle", nullable=False)
    ai_review_error: Mapped[str | None] = mapped_column(Text, nullable=True)
    ai_review_summary: Mapped[dict | None] = mapped_column(JSON, nullable=True)
    # Overview figure generated by AI
    overview_figure_url: Mapped[str | None] = mapped_column(Text, nullable=True)
    overview_figure_alt: Mapped[str | None] = mapped_column(Text, nullable=True)
    # Google Docs sync
